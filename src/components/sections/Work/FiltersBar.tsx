...
const FiltersBar = ({
  ...
  return (
    <ul className="mb-[clamp(2.4rem,6vw,12rem)] flex flex-wrap gap-[clamp(0.7rem,1.4vw,4rem)]">
      {FILTERS.map((filter, index) => (
        ...
        >
          <button
            className={cx(
              'flex h-12 items-center justify-center rounded-full border-[0.1vw] px-4 text-sm transition-all duration-500 sm:h-[clamp(3rem,4vw,10rem)] sm:px-[clamp(2rem,2vw,6rem)] sm:text-default',
