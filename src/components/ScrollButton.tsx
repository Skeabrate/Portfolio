...
const ScrollButton = ({ label, href }: { label: string; href: TRoute }) => {
  return (
    <motion.a
      ...
      className="group relative flex w-fit items-end gap-3 py-1 font-bold text-teal-500"
      href={'#' + href}
    >
      <ArrowSVG />
      {label}
      <motion.span
        ...
        className="absolute bottom-0 left-0 h-px w-2/3 origin-left bg-teal-500 transition-all duration-300 group-hover:w-full"
      />
    </motion.a>
  );
};
